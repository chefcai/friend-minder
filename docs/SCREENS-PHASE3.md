...
- **Removing members uses selection mode, not a per-row control** (FRM-167, GD-1; supersedes the earlier "48dp remove button" note). Same pattern as Home (Section 10): long-press a member, or use the row's "Select" accessibility action, to enter the mode. Tap then toggles; the header shows "N selected" with an X in the leading slot (X, system back, or deselecting the last member exits); the edit pencil hides. The FAB becomes "Remove from group" (person-remove icon) and **stays `fm_primary`**: leaving a group destroys nothing, unlike Home's red bulk stop-tracking. Removal is immediate and raises a 4-second **"N removed" Undo snackbar** anchored above the FAB; Undo re-adds the members and scrolls the first one into view. No confirmation dialog.
- Add member: FAB (§5.0).
...
- Segmented control **History / Special Dates** (`Widget.FM.SegmentedButton`, revised 2026-09-23 for audit CD-4 / FRM-159): selected segment is an `fm_primary` fill with a white label and a 16dp check; unselected is `fm_surface` with an `fm_ink` label; 1dp `fm_outline` border. The earlier `fm_primary_container` selected fill measured 1.35:1 against white (the shipped #D6EBF8 was 1.23:1), which fails WCAG 1.4.11's 3:1 for a state indicator; `fm_primary` is 4.67:1. The same component is used for Settings' Fixed time / Random window.
...
- **Group sheets on the shared base** (FRM-174, DL-2). New/Edit group, Add contact to group, Contact Detail's Groups sheet and the add-flow group picker all extend `FmBottomSheet`, so their titles share one style and each has a single full-width primary. New/Edit group: the 8 swatches are a **2 × 4 grid** of 48dp circles, 24dp apart with 16dp between rows. The selected swatch gets a **2dp `fm_ink` ring with a 3dp gap outside it**, not an `fm_primary` stroke on the swatch. A disabled "Create group" says "Name the group to create it".
