...
- Stats section pulls directly from Architect's `StatisticsService` (per PRD §7 Statistics entity) — Designer spec assumes fields `lastContacted`, `streak`, `reachRate` are available synchronously or via cached Flow; if Architect's service is async-only with no cache-then-refresh pattern, this section needs a skeleton state (flag to Architect/Tracker if that's not already covered — see FRM-38 dependency). **Resolved — see §7 item 2:** it's neither. Show a brief skeleton on cold cache / after `forceRefresh`; treat it as effectively instant otherwise.
...
2. **Stats section sync/async contract** — flagged in §4; needs a one-line confirmation from Architect (FRM-32/Statistics service) on whether Designer's "instant display, background refresh" assumption holds, or whether a loading skeleton is mandatory on every Contact Detail open.
   ...
   **Answered by Architect (2026-09-17, FRM-53):** `StatisticsService.getStatistics()`/`getAggregateStatistics()` (merged in PR #44, `domain/services/DefaultStatisticsService.kt`) are plain `suspend` functions with a 24h cache — there's no cached-value-now/refresh-in-background split. On a cache hit it returns immediately (a `SharedPreferences` read, which Android keeps fully in memory after the file's first load, so this is fast even across 50 contacts). On a cache miss or stale entry (or `forceRefresh = true`, e.g. right after a new outreach log), it computes synchronously within that same suspend call before returning — the math itself is pure and sub-millisecond (see `StatisticsCalculatorTest`), so in practice this is still effectively instant, but it is *not* a guaranteed-non-blocking API the way "background refresh" implies.

   Practical guidance for Publisher: call it from `viewModelScope` (not the main thread directly, though nothing here does real I/O beyond a `SharedPreferences` read). A loading skeleton isn't required for correctness, but showing one briefly on cold cache / after a `forceRefresh` call is the safer choice, since "instant" here is a likely characteristic of the current implementation and data scale, not a documented guarantee of the contract. If a hard non-blocking guarantee is wanted later (e.g. a `Flow<ContactStatistics>` that emits the stale cached value immediately and a fresh one after computing), that's a real API change from what's built — flag it if Publisher needs it, rather than assuming it's already there.

   Also worth noting for whoever reads this via FRM-53: the reach-rate example in this doc ("65% (13 of 20)") matches the PRD §6.5 definition exactly, and that's precisely what `StatisticsService` computes (`outreachCount / remindersSent`) — no discrepancy to reconcile there.
